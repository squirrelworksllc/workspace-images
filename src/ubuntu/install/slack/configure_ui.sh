...
# configure_ui.sh
#
# Purpose: Configures UI elements, sandboxing, and shortcuts for Slack.
###############################################################################
set -euo pipefail
...
if [ -f "$DESKTOP_FILE" ]; then
    ...
    # Slack is an Electron app; often needs no-sandbox in restricted containers
    ...
    cp "$DESKTOP_FILE" "$KASM_HOME/Desktop/slack.desktop"
    chmod +x "$KASM_HOME/Desktop/slack.desktop"
    chown 1000:1000 "$KASM_HOME/Desktop/slack.desktop"
    ...
    # We pre-emptively create a 'hidden' entry to block Slack from auto-starting.
    # Note: EOF MUST be at the beginning of the line.
...
EOF
    ...
    log "Slack UI configuration complete."
else
    log "WARNING: slack.desktop not found at $DESKTOP_FILE."
fi
